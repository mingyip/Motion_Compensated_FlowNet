...
class upsample_conv2d_and_predict_flow(nn.Module):
    ...
    def forward(self, conv):
        ...
        flow_y = self.flow_ratio * flow.shape[2] * flow[:, 0]
        flow_output = torch.cat([flow_x.unsqueeze(1), flow_y.unsqueeze(1)], 1)

        return torch.cat([conv,flow.clone()], dim=1), flow_output
